...
public class EstadoPedidoController {
    ...
    @PostMapping("/crear")
    public ResponseEntity<String> saveCategoria(@RequestBody EstadoPedido estadoPedido){
        estadoPedidoService.saveEstadoPedido(estadoPedido);
        return ResponseEntity.ok("Estado Pedido creada con éxito.");
    }

    @DeleteMapping("/borrar/{id}")
    public ResponseEntity<String> deleteEstado(@PathVariable Long id){
        estadoPedidoService.deleteEstadoPedido(id);
        return ResponseEntity.ok("El estado se eliminó con éxito.");
    }

    @PutMapping("/editar/{id}")
    public ResponseEntity<String> editEstado(@PathVariable Long id,
                                                @RequestBody EstadoPedido estadoPedidoUpdated){
